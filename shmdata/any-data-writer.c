...
{
    shmdata_any_writer_t *writer = (shmdata_any_writer_t *)g_malloc0 (sizeof(shmdata_any_writer_t));
    writer->debug_ = SHMDATA_DISABLE_DEBUG;
    g_log_set_default_handler (shmdata_any_writer_log_handler,
			       writer);
    writer->type_ = NULL;
...
void 
shmdata_any_writer_start (shmdata_any_writer_t *writer,const char *socketName)
{
    ...
    writer->base_writer_ = shmdata_base_writer_init (socketName,writer->pipeline_,writer->id_);
...
void 
shmdata_any_writer_set_data_type(shmdata_any_writer_t *writer,const char *type)
{
