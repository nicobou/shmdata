...
bool Segment::register_shmdata(ShmdataReader::ptr reader) {
  ...
  {  // JSON
    ...
    quid_->signal_emit("on-new-shmdata-reader",
                       ...
                       JSONBuilder::get_string(reader->get_json_root_node(),
                                               true).c_str());
  }
  return true;
}
...
bool Segment::unregister_shmdata(std::string shmdata_path) {
  ...
  if (update_reader) {
    update_shmdata_readers_description();
    quid_->prune_tree(std::string(".shmdata.reader.") + shmdata_path);
    segment_custom_props_->notify_property_changed(json_readers_description_);
  }
  ...
  if (update_writer) {
    update_shmdata_writers_description();
    quid_->prune_tree(std::string(".shmdata.writer.") + shmdata_path);
    segment_custom_props_->notify_property_changed(json_writers_description_);
  }
...
void Segment::update_shmdata_writers_description() {
...
void Segment::update_shmdata_readers_description() {
...
void Segment::populate_tree(std::string key, std::string caps) {
  std::string category;
...
Segment::install_connect_method(OnConnect on_connect_cb,
                                ...
                                uint max_reader) {
  if (quid_ == nullptr) {
    g_warning("Segment not initialized yet, call init_segment(this) first");
    return false;
  }
