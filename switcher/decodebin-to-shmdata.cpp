...
{
  DecodebinToShmdata::
    DecodebinToShmdata (GPipe & gpipe):decodebin_ ("decodebin2"),
...
//g_debug ("event probed (%s)\n", gst_structure_get_name (s));
    ...
    DecodebinToShmdata::pad_to_shmdata_writer (GstElement * bin, GstPad * pad)
  {
	...
	padname =
	  gst_structure_get_name (gst_caps_get_structure (padcaps, 0));
    }
	...
	media_name = padname_splitted[0];
      media_name.append ("-" +
			 std::to_string (gpipe_->get_count (media_name)));
