    ...
    else
      {
        switch (res)
          ...
          case GST_PAD_LINK_WAS_LINKED:
            g_debug
              ("GstUtils::check_pad_link_return - GST_PAD_LINK_WAS_LINKED");
            break;
          ...
          case GST_PAD_LINK_NOFORMAT:
            g_debug
              ("GstUtils::check_pad_link_return - GST_PAD_LINK_NOFORMAT");
            break;
          case GST_PAD_LINK_NOSCHED:
            g_debug
              ("GstUtils::check_pad_link_return - GST_PAD_LINK_NOSCHED");
            break;
          case GST_PAD_LINK_REFUSED:
            g_debug
              ("GstUtils::check_pad_link_return - GST_PAD_LINK_REFUSED");
            break;
          default:
  ...
  void GstUtils::clean_element (GstElement * element)
  {
    if (element != nullptr
        ...
        && GST_STATE_CHANGE_FAILURE != GST_STATE_RETURN (element))
      {
        ...
        gst_iterator_foreach (pad_iter, (GFunc) GstUtils::unlink_pad,
                              element);
...
//warning this may be blocking
        ...
        if (GST_IS_BIN (gst_element_get_parent (element)))
          gst_bin_remove (GST_BIN (gst_element_get_parent (element)),
                          element);
      }
    element = nullptr;
  }
        ...
        for (child = children; child != nullptr; child = g_list_next (child))
          {
            ...
            GstElementFactory *factory =
              gst_element_get_factory (current_element);
                                    ...
                                    const char *property_name)
  {
    ...
    if (pspec_master->value_type != pspec_slave->value_type)
      {
        g_debug
          ("master and slave properties has different type, canont apply");
        return false;
      }
