...
//g_assert (descend != nullptr);
  ...
  std::string QuiddityManager_Impl::get_class_doc (std::string class_name)
  {
    if (abstract_factory_.key_exists (class_name))
      return JSONBuilder::get_string (abstract_factory_.
  ...
  bool QuiddityManager_Impl::init_quiddity (Quiddity::ptr quiddity)
  {
    ...
    quiddities_nick_names_[quiddity->get_nick_name ()] =
      quiddity->get_name ();

    if (creation_hook_ != nullptr)
      (*creation_hook_) (quiddity->get_nick_name (),
			 creation_hook_user_data_);
  ...
  std::string QuiddityManager_Impl::get_quiddities_description ()
  {
	...
	descr->add_string_member ("category",
				  quid->get_documentation ().get_category ().
				  c_str ());
	descr->add_string_member ("long name",
				  quid->get_documentation ().get_long_name ().
				  c_str ());
	descr->add_string_member ("description",
				  quid->get_documentation ().
				  get_description ().c_str ());
	descr->add_string_member ("license",
				  quid->get_documentation ().get_license ().
				  c_str ());
	descr->add_string_member ("author",
				  quid->get_documentation ().get_author ().
				  c_str ());
	descr->end_object ();
    ...
    QuiddityManager_Impl::get_quiddity_description (std::string nick_name)
  {
    ...
    descr->add_string_member ("category",
			      quiddities_[quiddities_nick_names_[nick_name]]->
			      get_documentation ().get_category ().c_str ());
    descr->add_string_member ("long name",
			      quiddities_[quiddities_nick_names_[nick_name]]->
			      get_documentation ().get_long_name ().c_str ());
    descr->add_string_member ("description",
			      quiddities_[quiddities_nick_names_[nick_name]]->
			      get_documentation ().get_description ().
			      c_str ());
    descr->add_string_member ("license",
			      quiddities_[quiddities_nick_names_[nick_name]]->
			      get_documentation ().get_license ().c_str ());
    descr->add_string_member ("author",
			      quiddities_[quiddities_nick_names_[nick_name]]->
			      get_documentation ().get_author ().c_str ());
    descr->end_object ();
    return descr->get_string (true);
  }
  ...
  bool QuiddityManager_Impl::exists (std::string quiddity_name)
  {
    return (quiddities_nick_names_.end () !=
  ...
  std::string
    QuiddityManager_Impl::get_properties_description (std::
						      string quiddity_name)
  {
						    ...
						    std::string property_name)
  {
    ...
    return (get_quiddity (quiddity_name))->
      get_property_description (property_name);
  }

  std::string
    QuiddityManager_Impl::get_properties_description_by_class (std::
							       string
							       class_name)
  {
    ...
    if (g_strcmp0 (quid_name.c_str (), "") == 0)
      return
	"{\"error\":\"cannot get property because the class cannot be instanciated\"}";
  ...
  std::string
    QuiddityManager_Impl::get_property_description_by_class (std::
							     string
							     class_name,
							     std::
							     string
					...
					std::string property_value)
  {
    ...
    return (get_quiddity (quiddity_name))->set_property (property_name.
							 c_str (),
							 property_value.
							 c_str ());
  }
  ...
  bool
    QuiddityManager_Impl::make_property_subscriber (std::
  ...
  bool
    QuiddityManager_Impl::remove_property_subscriber (std::
						      string subscriber_name)
  {
					      ...
					      std::string property_name)
  {
    ...
    return property_subscribers_[subscriber_name]->
      subscribe (get_quiddity (quiddity_name), property_name);
  }
						...
						std::string property_name)
  {
    ...
    return property_subscribers_[subscriber_name]->
      unsubscribe (get_quiddity (quiddity_name), property_name);
  }
  ...
  std::vector < std::pair < std::string,
    std::string >
    >QuiddityManager_Impl::list_subscribed_properties (std::
						       string subscriber_name)
  {
    auto it = property_subscribers_.find (subscriber_name);
    if (property_subscribers_.end () == it)
  ...
  std::string
    QuiddityManager_Impl::list_subscribed_properties_json (std::
							   string
							   subscriber_name)
  {
						   ...
						   void *user_data)
  {
    ...
    return (get_quiddity (quiddity_name))->subscribe_property (property_name.
							       c_str (), cb,
							       user_data);
  }

  bool
    QuiddityManager_Impl::unsubscribe_property_glib (std::
						     string quiddity_name,
						     std::
						     ...
						     void *user_data)
  {
    ...
    return (get_quiddity (quiddity_name))->
      unsubscribe_property (property_name.c_str (), cb, user_data);
  }
					...
					std::string property_name)
  {
    ...
    return (get_quiddity (quiddity_name))->get_property (property_name.
							 c_str ());
  }
				  ...
				  const std::vector < std::string > args)
  {
    ...
    if (!exists (quiddity_name))
      {
	g_debug ("quiddity %s not found, cannot invoke",
		 quiddity_name.c_str ());
						  ...
						  std::string method_name)
  {
    ...
    return (get_quiddity (quiddity_name))->
      get_method_description (method_name);
  }

  std::string
    QuiddityManager_Impl::get_methods_description_by_class (std::
							    string class_name)
  {
    if (!class_exists (class_name))
      return "{\"error\":\"class not found\"}";
  ...
  std::string
    QuiddityManager_Impl::get_method_description_by_class (std::
							   string class_name,
							   std::
							   string method_name)
  {
						  ...
						  std::string signal_name)
  {
    ...
    return (get_quiddity (quiddity_name))->
      get_signal_description (signal_name);
  }

  std::string
    QuiddityManager_Impl::get_signals_description_by_class (std::
							    string class_name)
  {
    if (!class_exists (class_name))
      return "{\"error\":\"class not found\"}";
    ...
    if (g_strcmp0 (quid_name.c_str (), "") == 0)
      return
	"{\"error\":\"cannot get signal description because the class cannot be instanciated\"}";
  ...
  std::string
    QuiddityManager_Impl::get_signal_description_by_class (std::
							   string class_name,
							   std::
							   string signal_name)
  {
    ...
    if (g_strcmp0 (quid_name.c_str (), "") == 0)
      return
	"{\"error\":\"cannot get signal because the class cannot be instanciated\"}";
  ...
  bool
    QuiddityManager_Impl::remove_signal_subscriber (std::
						    string subscriber_name)
  {
    auto it = signal_subscribers_.find (subscriber_name);
    if (signal_subscribers_.end () == it)
					    ...
					    std::string signal_name)
  {
    ...
    return signal_subscribers_[subscriber_name]->
      subscribe (get_quiddity (quiddity_name), signal_name);
  }
					      ...
					      std::string signal_name)
  {
    ...
    return signal_subscribers_[subscriber_name]->
      unsubscribe (get_quiddity (quiddity_name), signal_name);
  }
  ...
  std::vector < std::pair < std::string,
    std::string >
    >QuiddityManager_Impl::list_subscribed_signals (std::
						    string subscriber_name)
  {
    auto it = signal_subscribers_.find (subscriber_name);
    if (signal_subscribers_.end () == it)
  ...
  std::string
    QuiddityManager_Impl::list_subscribed_signals_json (std::
							string
							subscriber_name)
  {
  ...
  bool
    ...
    scan_directory_for_plugins (const char *directory_path)
  {
