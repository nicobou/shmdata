...
ShmdataDecoder::ShmdataDecoder(Quiddity* quid,
  ...
  // HERE link shmdatasrc with the decodebin, as done in httpsdpdec, then use it
  // in pj-call
  ...
  //   decodebin->set_media_label(gst_structure_get_string (structure,
  //   "media-label"));
  if (!media_label_.empty()) decodebin_.set_media_label(media_label_);
  g_print("%s %d\n", __FUNCTION__, __LINE__);
...
void ShmdataDecoder::configure_shmdatasink(GstElement* element,
  ...
  if (shm_prefix_.empty())
    shmpath = quid_->get_file_name_prefix() + quid_->get_manager_name() + "_" +
              quid_->get_name() + "_";
  else
    shmpath = shm_prefix_;
  ...
  if (on_shmwriter_path_cb_) on_shmwriter_path_cb_(shmwriter_path_);
  g_print("%s %d\n", __FUNCTION__, __LINE__);
