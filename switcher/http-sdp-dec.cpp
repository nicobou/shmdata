...
void HTTPSDPDec::init_httpsdpdec() {
  ...
  g_object_set(G_OBJECT(sdpdemux_), "latency", 0, nullptr);
  g_object_set(G_OBJECT(sdpdemux_), "async-handling", TRUE, nullptr);
...
void HTTPSDPDec::uri_to_shmdata() {
  ...
  g_print("%s line %d\n", __FUNCTION__, __LINE__);
  g_object_set(G_OBJECT(souphttpsrc_), "location", uri_.c_str(), nullptr);
  g_print("%s line %d\n", __FUNCTION__, __LINE__);
