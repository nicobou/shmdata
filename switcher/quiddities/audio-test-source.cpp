...
bool AudioTestSource::start() {
  if (!gst_pipeline_) {
    sw_warning("BUG: gst_pipeline failed to be created, something went very wrong! (audiotestsrc)");
    return false;
  }
...
bool AudioTestSource::stop() {
  ...
  if (!gst::UGstElem::renew(audiotestsrc_, {"is-live", "samplesperbuffer"}) ||
    ...
    sw_warning("error initializing gst element for audiotestsrc");
    gst_pipeline_.reset();
    return false;
  }
