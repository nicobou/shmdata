...
namespace switcher
{
  ...
  Uridecodebin::Uridecodebin ():uridecodebin_ (nullptr),
    media_counters_ (),
    main_pad_ (nullptr),
    rtpgstcaps_ (nullptr),
    ...
    playing_ (true), uri_spec_ (nullptr), uri_ (g_strdup (""))
  {
  }
  ...
  gboolean sink_factory_filter (GstPluginFeature * feature, gpointer data)
  {
	...
	GstStaticPadTemplate *padTemplate =
	  (GstStaticPadTemplate *) item->data;
  ...
  void Uridecodebin::pad_to_shmdata_writer (GstElement * bin, GstPad * pad)
  {
    ...
    g_object_set (fakesink, "sync", TRUE, "signal-handoffs", TRUE, nullptr);
    GstElement *funnel;
    ...
    g_signal_connect (fakesink, "handoff", (GCallback) on_handoff_cb,
		      connector.get ());
...
//adding a probe for discarding uncomplete packets
	...
	gst_pad_add_buffer_probe (depaysrcpad,
				  G_CALLBACK (Uridecodebin::
					      gstrtpdepay_buffer_probe_cb),
				  context);
...
//adding a probe for handling loss messages from rtpbin
	gst_pad_add_event_probe (sinkpad,
				 (GCallback) Uridecodebin::
				 gstrtpdepay_event_probe_cb, context);
