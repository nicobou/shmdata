...
namespace switcher
{
  ...
  JackSink::make_elements ()
  {
    ...
    gchar *description = g_strdup_printf ("audioconvert ! audioresample ! queue max-size-buffers=2 leaky=downstream ! jackaudiosink provide-clock=false slave-method=none client-name=%s sync=false", client_name_);
