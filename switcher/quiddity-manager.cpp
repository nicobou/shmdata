...
namespace switcher {
  ...
  QuiddityManager::CommandHistory
    ...
    for (i = 0; i < num_elements; i++) {
      ...
      res.push_back (QuiddityCommand::parse_command_from_json_reader
                     (reader));
    }
...
//invocation_thread_ = g_thread_new ("invocation_thread", GThreadFunc(invocation_thread), this);
  ...
  gboolean QuiddityManager::execute_command (gpointer user_data) {
    ...
    case QuiddityCommand::get_class_doc:
      context->command_->result_.push_back (context->manager_impl_->
                                            get_class_doc (context->command_->
                                                           args_[0]));
      break;
    ...
    case QuiddityCommand::get_methods_description:
      context->command_->result_.push_back (context->manager_impl_->
                                            get_methods_description (context->
                                                                     command_->
                                                                     args_
                                                                     [0]));
      break;
    case QuiddityCommand::get_method_description:
      context->command_->result_.push_back (context->manager_impl_->
                                            get_method_description (context->
                                                                    command_->
                                                                    args_[0],
                                                                    context->
                                                                    command_->
