...
namespace switcher
{
  ...
  QuiddityManager::QuiddityManager (std::
				    string
				    name):manager_impl_
    (QuiddityManager_Impl::make_manager (name)), name_ (name), command_ (),
    seq_mutex_ (), command_queue_ (g_async_queue_new ()),
    invocation_thread_ (std::
			thread (&QuiddityManager::invocation_thread, this)),
  ...
  void
    QuiddityManager::play_command_history (QuiddityManager::
					   CommandHistory histo,
					   QuiddityManager::PropCallbackMap *
					   prop_cb_data,
					   QuiddityManager::
					   SignalCallbackMap * sig_cb_data,
  ...
  std::vector < std::string >
    QuiddityManager::get_property_subscribers_names (QuiddityManager::
						     CommandHistory histo)
  {
  ...
  std::vector < std::string >
    QuiddityManager::get_signal_subscribers_names (QuiddityManager::
						   CommandHistory histo)
  {
    std::vector < std::string > res;
  for (auto & it:histo)
    ...
    QuiddityManager::get_command_history_from_file (const char *file_path)
  {
	...
	res.
	  push_back (QuiddityCommand::
		     parse_command_from_json_reader (reader));
      }
  ...
  std::string
    QuiddityManager::get_properties_description_by_class (std::
							  string class_name)
  {
  ...
  std::string
    QuiddityManager::get_property_description_by_class (std::
							string class_name,
							std::
  ...
  std::string
    QuiddityManager::list_subscribed_properties_json (std::
  ...
  std::string
    QuiddityManager::list_subscribed_signals_json (std::
						   string subscriber_name)
  {
    command_lock ();
						      ...
						      std::string signal_name)
  {
    ...
    command_->add_arg (signal_name);
    std::string res =
      manager_impl_->get_signal_description_by_class (class_name,
						      signal_name);
    command_->result_.push_back (res);
    command_unlock ();
    return res;
...
//invocation_thread_ = g_thread_new ("invocation_thread", GThreadFunc(invocation_thread), this);
  ...
  gboolean QuiddityManager::execute_command (gpointer user_data)
  {
	...
	context->command_->result_.push_back (context->manager_impl_->
					      get_classes_doc ());
	break;
      case QuiddityCommand::get_class_doc:
	context->command_->result_.push_back (context->manager_impl_->
					      get_class_doc (context->
							     command_->
							     args_[0]));
	break;
	...
	if (context->manager_impl_->
	    rename (context->command_->args_[0], context->command_->args_[1]))
	...
	if (context->manager_impl_->
	    set_property (context->command_->args_[0],
			  context->command_->args_[1],
	...
	if (context->manager_impl_->
	    subscribe_signal (context->command_->args_[0],
			      context->command_->args_[1],
			      context->command_->args_[2]))
	  context->command_->result_.push_back ("true");
	else
	...
	if (context->manager_impl_->
	    unsubscribe_signal (context->command_->args_[0],
				context->command_->args_[1],
				context->command_->args_[2]))
	  context->command_->result_.push_back ("true");
	else
	...
	if (context->manager_impl_->
	    scan_directory_for_plugins (context->command_->args_[0].c_str ()))
	  context->command_->result_.push_back ("true");
	else
	  context->command_->result_.push_back ("false");
	break;
	...
	if (context->manager_impl_->
	    subscribe_property (context->command_->args_[0],
				context->command_->args_[1],
				context->command_->args_[2]))
	  context->command_->result_.push_back ("true");
	else
	...
	if (context->manager_impl_->
	    unsubscribe_property (context->command_->args_[0],
				  context->command_->args_[1],
				  context->command_->args_[2]))
	  context->command_->result_.push_back ("true");
	else
	...
	g_debug ("** unknown command, cannot launch %s\n",
		 QuiddityCommand::get_string_from_id (context->command_->
						      id_));
      }
