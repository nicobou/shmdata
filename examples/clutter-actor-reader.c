...
{
  ClutterActor *rect_actor =
    (ClutterActor *) g_object_get_data (G_OBJECT (timeline), "rect_actor");
  if (rect_actor != NULL)
...
update_texture_actor (gpointer data)
{
  ...
  GAsyncQueue *queue_input_buf =
    (GAsyncQueue *) g_object_get_data (G_OBJECT (texture_actor),
				       "queue_input_buf");
  GAsyncQueue *queue_output_buf =
    (GAsyncQueue *) g_object_get_data (G_OBJECT (texture_actor),
				       "queue_output_buf");
  GstGLBuffer *gst_gl_buf =
    (GstGLBuffer *) g_async_queue_pop (queue_input_buf);
