...
NVencES::NVencES(logger::Logger* logger, uint32_t device_id) : logger_(logger), cu_ctx_(device_id) {
  On_scope_exit {
    if (!safe_bool_idiom()) logger_->sw_warning("NV encoder session initialization failed");
  };
  if (!cu_ctx_) return;
...
NVencES::~NVencES() {
  ...
  if (safe_bool_idiom() && !(NV_ENC_SUCCESS == NVencAPI::api.nvEncDestroyEncoder(encoder_)))
    logger_->sw_warning("BUG! (destroying NV encoder session)");
}
...
NVencES::named_guid_t NVencES::get_supported_codecs() {
  ...
  if (NV_ENC_SUCCESS !=
    ...
    logger_->sw_warning("issue with nvEncGetEncodeGUIDs");
    return res;
  }
...
NVencES::named_guid_t NVencES::get_presets(GUID encodeGUID) {
  ...
  if (NV_ENC_SUCCESS !=
    ...
    logger_->sw_warning("issue with nvEncGetEncodePresetGUIDs");
    return res;
  }
  for (uint32_t i = 0; i < presets_num_; ++i) {
    if (is_same(presets_guids_[i], NV_ENC_PRESET_DEFAULT_GUID)) {
      ...
      logger_->sw_debug("unknown preset GUID from nvenc");
  }
  return res;
}
...
NVencES::named_guid_t NVencES::get_profiles(GUID encodeGUID) {
  ...
  for (uint32_t i = 0; i < profiles_num_; ++i) {
    if (is_same(profiles_guids_[i], NV_ENC_CODEC_PROFILE_AUTOSELECT_GUID)) {
      ...
      logger_->sw_debug("unknown profile GUID from nvenc");
  }
  return res;
}
  ...
  for (uint32_t i = 0; i < input_formats_num_; ++i) {
    switch (buf_formats_[i]) {
      case NV_ENC_BUFFER_FORMAT_UNDEFINED:
        logger_->sw_warning("nvEncGetInputFormats gives NV_ENC_BUFFER_FORMAT_UNDEFINED (?)");
        break;
      ...
      case NV_ENC_BUFFER_FORMAT_AYUV:
        ...
        break;
      default:
        logger_->sw_debug("unknown input format from nvenc");
    }
  }
  return res;
}
...
bool NVencES::initialize_encoder(GUID encodeGuid,
  ...
  if (NV_ENC_SUCCESS !=
    ...
    logger_->sw_warning("nvenc cannot get encode preset config");
  } else {
    preset_config_.presetCfg.version = NV_ENC_CONFIG_VER;
    ...
    if (is_same(encodeGuid, NV_ENC_CODEC_H264_GUID)) {
      ...
      logger_->sw_warning("Unknown codec for hardware encoding.");
      return false;
    }
  ...
  if (NV_ENC_SUCCESS != status) {
    logger_->sw_warning("encode session initialization failed");
    return false;
  }
