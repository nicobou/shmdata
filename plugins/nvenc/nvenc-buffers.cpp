...
NVencBuffers::NVencBuffers(void* encoder,
  ...
  for (auto& it : input_bufs_) {
    ...
    if (NV_ENC_SUCCESS !=
        NVencAPI::api.nvEncCreateInputBuffer(encoder_, &buf)) {
  ...
  for (auto& it : output_bufs_) {
    ...
    buf.memoryHeap = NV_ENC_MEMORY_HEAP_SYSMEM_CACHED;
    if (NV_ENC_SUCCESS !=
        NVencAPI::api.nvEncCreateBitstreamBuffer(encoder_, &buf)) {
...
NVencBuffers::~NVencBuffers() {
  ...
  for (auto& it : output_bufs_)
    if (nullptr != it &&
        NV_ENC_SUCCESS !=
            NVencAPI::api.nvEncDestroyBitstreamBuffer(encoder_, it))
      g_warning("nvenc cannot destroy output buffer");
}

bool NVencBuffers::safe_bool_idiom() const {
  return input_bufs_.end() ==
             std::find(input_bufs_.begin(), input_bufs_.end(), nullptr) &&
         output_bufs_.end() ==
             std::find(output_bufs_.begin(), output_bufs_.end(), nullptr);
}
...
bool NVencBuffers::copy_to_next_input_buffer(void* data, size_t size) {
  ...
  if (NV_ENC_SUCCESS !=
      NVencAPI::api.nvEncLockInputBuffer(encoder_, &lockInputBufferParams))
    return false;
  if (format_ == NV_ENC_BUFFER_FORMAT_NV12_PL) {
    ...
    for (guint y = 0; y < height_ / 2; ++y) {
      ...
      src += src_stride;
             ...
             format_ == NV_ENC_BUFFER_FORMAT_IYUV_PL) {
    guint8* src = (guint8*)data;
    guint src_stride = width_;
    guint8* dest = (guint8*)lockInputBufferParams.bufferDataPtr;
    guint dest_stride = lockInputBufferParams.pitch;
...
bool NVencBuffers::encode_current_input() {
  ...
  NVENCSTATUS nvStatus =
      NVencAPI::api.nvEncEncodePicture(encoder_, &encPicParams);
...
bool NVencBuffers::process_encoded_frame(
    std::function<void(void*, uint32_t)> fun) {
  ...
  NVENCSTATUS nvStatus =
      NVencAPI::api.nvEncLockBitstream(encoder_, &lockBitstreamData);
  if (nvStatus == NV_ENC_SUCCESS) {
    fun(lockBitstreamData.bitstreamBufferPtr,
