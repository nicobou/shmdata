...
namespace switcher
{
  ...
  SoapCtrlServer::SoapCtrlServer ():soap_ (),
    port_ (8080),
    quit_server_thread_ (false),
    service_ (nullptr), socket_ (), thread_ (), mutex_ ()
  {
  }
  ...
  int SoapCtrlServer::http_get (struct soap *soap)
  {
	...
	while (query_vars[i] != nullptr)
	  {
	    ...
	    else if (g_strcmp0 (var[0], "destination") == 0
		     && var[1] != nullptr)
	      {
	...
	if (!manager->
	    invoke (rtpsession_name, "write_sdp_file", nullptr, arg))
	  return 404;
	...
	sdp_file.append (manager->get_name () + "_" + rtpsession_name + "_" +
	...
	if (g_file_get_contents (sdp_file.c_str (), &sdp_contents, &file_length, nullptr) && 0 != file_length)	//not getting errors
	  ...
	  g_warning
	    ("cannot get sdp description for rtpsession %s, destination %s\n",
	     rtpsession_name.c_str (), destination_name.c_str ());
      }
    return 404;
  }
...
{				//FIXME rename that to get_classes
...
int
controlService::get_quiddity_description (std::string quiddity_name,
					  std::string * result)
{
  using namespace switcher;
  SoapCtrlServer *ctrl_server = (SoapCtrlServer *) this->user;
						   ...
						   std::string * result)
{
  ...
  *result =
    manager->get_property_description_by_class (class_name, property_name);
				 ...
				 std::string * result)
{
  ...
  if (name != "")
    ...
    {
      char *s = (char *) soap_malloc (this, 1024);
      sprintf (s, "%s cannot be created, see switcher logs",
						 ...
						 std::string * result)
{
  ...
  *result =
    manager->get_method_description_by_class (class_name, method_name);
  return SOAP_OK;
}
						 ...
						 std::string * result)
{
  ...
  *result =
    manager->get_signal_description_by_class (class_name, signal_name);
  return SOAP_OK;
}

int
controlService::save (std::string file_name, std::string * result)
{
  using namespace switcher;
...
int
controlService::load (std::string file_name, std::string * result)
{
  using namespace switcher;
...
int
controlService::run (std::string file_name, std::string * result)
{
  using namespace switcher;
