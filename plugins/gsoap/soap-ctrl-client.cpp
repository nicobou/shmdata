...
bool SoapCtrlClient::init() {
  ...
  pmanage<MPtr(&PContainer::make_string)>("url",
                                          nullptr,
                                          [this]() { return url_; },
                                          "Remote server URL",
                                          "Remote server URL",
  ...
  install_method(
      "Set Remote Switcher URL And Retry Until Success",
...
gboolean SoapCtrlClient::set_remote_url_retry(gpointer url,
                                              gpointer user_data) {
  SoapCtrlClient* context = static_cast<SoapCtrlClient*>(user_data);
  context->reset_endpoint();
  context->url_ = std::string((const char*)url);
  ...
  if (TRUE == context->try_connect(context)) {
    context->try_connect_g_source_ = GstUtils::g_timeout_add_to_context(
...
gboolean SoapCtrlClient::create(gpointer class_name,
