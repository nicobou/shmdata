    ...
    while (running_)
      {
            ...
            if (core.substr (0, 3) == "cpu")
              {
                long
                  totalTime =
                  user + nice + system + idle + io + irq + softIrq + steal +
                  guest;
                ...
                if (firstRun)
                ...
                else
                  {
                    ...
                    float
                      ...
                      idleP;
                    userP =
                      (float) (user - _cpus[core].user) / (float) (totalTime -
                                                                   _cpus
                                                                   [core].totalTime);
                    niceP =
                      (float) (nice - _cpus[core].nice) / (float) (totalTime -
                                                                   _cpus
                                                                   [core].totalTime);
                    systemP =
                      (float) (system -
                               _cpus[core].system) / (float) (totalTime -
                                                              _cpus
                                                              [core].totalTime);
                    idleP =
                      (float) (idle - _cpus[core].idle) / (float) (totalTime -
                                                                   _cpus
                                                                   [core].totalTime);
        ...
        for (string line; getline (file, line);)
          {
            ...
            if (netI.find ("Inter") == string::npos
                && netI.find ("face") == string::npos)
              {
                ...
                if (firstRun)
                  {
                    tree_->graft (".net." + netName + ".rx_rate",
                                  make_tree ());
                    tree_->graft (".net." + netName + ".rx_bytes",
                                  make_tree ());
                    tree_->graft (".net." + netName + ".rx_packets",
                                  make_tree ());
                    tree_->graft (".net." + netName + ".rx_errors",
                                  make_tree ());
                    tree_->graft (".net." + netName + ".rx_drop",
                                  make_tree ());

                    tree_->graft (".net." + netName + ".tx_rate",
                                  make_tree ());
                    tree_->graft (".net." + netName + ".tx_bytes",
                                  make_tree ());
                    tree_->graft (".net." + netName + ".tx_packets",
                                  make_tree ());
                    tree_->graft (".net." + netName + ".tx_errors",
                                  make_tree ());
                    tree_->graft (".net." + netName + ".tx_drop",
                                  make_tree ());
                  ...
                  {
                    long
                      rx_delta = rBytes - _net[netName].rx_bytes;
                    long
                      tx_delta = tBytes - _net[netName].tx_bytes;

                    _net[netName].rx_rate =
                      (long) ((double) rx_delta / period_);
                    _net[netName].tx_rate =
                      (long) ((double) tx_delta / period_);
        ...
        this_thread::
          sleep_for (chrono::milliseconds ((int) (period_ * 1000.0)));
      }
  }
