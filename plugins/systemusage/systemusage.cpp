...
void SystemUsage::pollState() {
  ...
  for (string line; getline(file, line);) {
    ...
    if (core.substr(0, 3) == "cpu") {
      ...
      tree_->branch_set_data(".cpu." + core + ".total", totalP);
      tree_->branch_set_data(".cpu." + core + ".user", userP);
      tree_->branch_set_data(".cpu." + core + ".nice", niceP);
      tree_->branch_set_data(".cpu." + core + ".system", systemP);
      tree_->branch_set_data(".cpu." + core + ".idle", idleP);
  ...
  for (string line; getline(file, line);) {
    ...
    if (type.find("MemTotal") != string::npos)
      tree_->branch_set_data("mem.total", qty);
    else if (type.find("MemFree") != string::npos)
      tree_->branch_set_data("mem.free", qty);
    else if (type.find("Buffers") != string::npos)
      tree_->branch_set_data("mem.buffers", qty);
    else if (type.find("Cached") != string::npos &&
             type.find("SwapCached") == string::npos)
      tree_->branch_set_data("mem.cached", qty);
    else if (type.find("SwapTotal") != string::npos)
      tree_->branch_set_data("mem.swap_total", qty);
    else if (type.find("SwapFree") != string::npos)
      tree_->branch_set_data("mem.swap_free", qty);
  }
  file.close();
  ...
  for (string line; getline(file, line);) {
    ...
    if (netI.find("Inter") == string::npos &&
      ...
      if (false) {  // FIXME
        ...
        tree_->branch_set_data(".net." + netName + ".rx_rate",
                               _net[netName].rx_rate);
        tree_->branch_set_data(".net." + netName + ".tx_rate",
      ...
      tree_->branch_set_data(".net." + netName + ".rx_bytes",
                             _net[netName].rx_bytes);
      tree_->branch_set_data(".net." + netName + ".rx_packets",
                             _net[netName].rx_packets);
      tree_->branch_set_data(".net." + netName + ".rx_errors",
                             _net[netName].rx_errors);
      tree_->branch_set_data(".net." + netName + ".rx_drop",
                             _net[netName].rx_drop);
      tree_->branch_set_data(".net." + netName + ".tx_bytes",
                             _net[netName].tx_bytes);
      tree_->branch_set_data(".net." + netName + ".tx_packets",
                             _net[netName].tx_packets);
      tree_->branch_set_data(".net." + netName + ".tx_errors",
                             _net[netName].tx_errors);
      tree_->branch_set_data(".net." + netName + ".tx_drop",
