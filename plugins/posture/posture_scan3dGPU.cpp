...
bool PostureScan3DGPU::start() {
    ...
    while (update_loop_started_) {
      ...
      if (mesh_serialized.size() > 0) {
        if (!mesh_writer_ ||
          ...
          string data_type = compress_multicore_ ? POLYGONMESH_TYPE_MULTIMESH
  ...
  bool PostureScan3DGPU::init() {
    ...
    pmanage<MPtr(&PContainer::make_parented_double)>(
        ...
        [this]() {
          return std::max(grid_size_[0],
                          std::max(grid_size_[1], grid_size_[2]));
        },
        "Mesh grid size",
    ...
    pmanage<MPtr(&PContainer::make_parented_int)>(
        ...
        [this](const int& val) {
          ...
          if (solidifyGPU_)
            solidifyGPU_->setDepthFiltering(kernel_filter_size_,
  ...
  void PostureScan3DGPU::cb_frame_depth(int index,
    ...
    if (lockCamera.owns_lock()) {
      // convert incoming depth vector to unsigned short vector. (There must be
      // a clean way to avoid doing the copy.)
      std::vector<unsigned short> depthUS(depth.size() / 2);
      memcpy(depthUS.data(), depth.data(), depth.size());

      solidifyGPU_->setInputDepthMap(index, depthUS, width, height);
