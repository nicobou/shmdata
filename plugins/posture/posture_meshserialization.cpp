...
bool
PostureMeshSerialization::connect(std::string shmdata_socket_path) {
  mesh_reader_ = std2::make_unique<ShmdataFollower>(this,
                  shmdata_socket_path,
                  [=] (void *data, size_t size) {
    unique_lock<mutex> lock(mutex_);

    if (serializer_ == nullptr || (mesh_reader_caps_ != string(POLYGONMESH_TYPE_BASE)))
      return;
    ...
    auto mesh = serializer_->deserializeAsTextured(vector<uint8_t>((uint8_t*)data, (uint8_t*)data + size), timestamp);
    ...
    if (serializedMesh.size() != 0) {
      ...
      mesh_writer_->writer<MPtr(&shmdata::Writer::copy_to_shm)>(const_cast<unsigned char*>(serializedMesh.data()), serializedMesh.size());
...
bool
PostureMeshSerialization::disconnect(std::string) {
  unique_lock<mutex> lock(mutex_);
  mesh_reader_.reset();
  return true;
}
...
bool
PostureMeshSerialization::disconnect_all() {
  disconnect("");
  return true;
}

bool
PostureMeshSerialization::can_sink_caps(std::string caps) {
