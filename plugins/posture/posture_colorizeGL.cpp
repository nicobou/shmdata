...
PostureColorizeGL::~PostureColorizeGL() {
  stop();
}

bool
PostureColorizeGL::start() {
  if (is_started())
    return false;
...
bool
PostureColorizeGL::connect(std::string shmdata_socket_path) {
...
bool
PostureColorizeGL::disconnect(std::string /*unused*/) {
  return true;
}

bool
PostureColorizeGL::disconnect_all() {
  return true;
}

bool
PostureColorizeGL::can_sink_caps(std::string caps) {
...
bool
PostureColorizeGL::check_image_caps(string caps, unsigned int& width, unsigned int& height, unsigned int& channels)
{
