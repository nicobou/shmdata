...
namespace switcher
{
  ...
  //floor
  gchar *OscCtrlServer::string_float_to_string_int (const gchar *
						    string_float)
  {
    gchar *res;
	...
	if (argc == 3)
	  {
	    ...
	    if (!manager->
		make_property_subscriber (internal_subscriber_name,
					  OscCtrlServer::prop_cb, context))
	      return 0;

	    g_debug ("subscriber %s, %s, %s", subscriber_name, host,
		     port_int);
	  ...
	  g_warning
	    ("OSCctl: add subscriber needs 3 args (name, host, port)");
	return 0;
      }
