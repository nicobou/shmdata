...
PJCall::PJCall() {
  ...
  for (auto& it : local_ips_)
    SIPPlugin::this_->sw_debug("local IP found for interface {}: {}", it.first, it.second);
  for (auto& it : local_ips_) {
  ...
  /*  Init invite session module. */
  {
    ...
    if (status != PJ_SUCCESS)
      SIPPlugin::this_->sw_warning("unregistering default invite module failed");
    /* Initialize invite session module:  */
    status = pjsip_inv_usage_init(PJSIP::this_->sip_endpt_, &inv_cb);
    if (status != PJ_SUCCESS) SIPPlugin::this_->sw_warning("init invite session module failed");
  }
  ...
  if (status != PJ_SUCCESS) SIPPlugin::this_->sw_warning("register mod_siprtp_ failed");
  // registering codecs
...
void PJCall::on_inv_state_confirmed(call_t* call, pjsip_inv_session* /*inv*/, pjsua_buddy_id id) {
  SIPPlugin::this_->sw_debug("call connected");
  // updating call status in the tree
  InfoTree::ptr tree =
      SIPPlugin::this_->prune_tree(std::string(".buddies." + std::to_string(id)),
                                   false);  // do not signal since the branch will be re-grafted
...
void PJCall::call_on_state_changed(pjsip_inv_session* inv, pjsip_event* /*e*/) {
  ...
  if (!call) {
    SIPPlugin::this_->sw_warning("{}, null call in invite", std::string(__FUNCTION__));
    return;
  }
  ...
  if (PJSUA_INVALID_ID == id) {
    SIPPlugin::this_->sw_warning("buddy not found: cannot update call status ({})", call->peer_uri);
    return;
  }
  switch (inv->state) {
    case PJSIP_INV_STATE_DISCONNECTED:
      SIPPlugin::this_->sw_debug("PJSIP_INV_STATE_DISCONNECTED");
      PJCall::on_inv_state_disconnected(call, inv, id);
      break;
    case PJSIP_INV_STATE_CONFIRMED:
      SIPPlugin::this_->sw_debug("PJSIP_INV_STATE_CONFIRMED");
      PJCall::on_inv_state_confirmed(call, inv, id);
      break;
    case PJSIP_INV_STATE_EARLY:
      SIPPlugin::this_->sw_debug("PJSIP_INV_STATE_EARLY");
      PJCall::on_inv_state_early(call, inv, id);
      break;
    case PJSIP_INV_STATE_CONNECTING:
      SIPPlugin::this_->sw_debug("PJSIP_INV_STATE_CONNECTING");
      PJCall::on_inv_state_connecting(call, inv, id);
      break;
    case PJSIP_INV_STATE_NULL:
      SIPPlugin::this_->sw_debug("PJSIP_INV_STATE_NULL");
      break;
    case PJSIP_INV_STATE_CALLING:
      SIPPlugin::this_->sw_debug("PJSIP_INV_STATE_CALLING");
      break;
    case PJSIP_INV_STATE_INCOMING:
      SIPPlugin::this_->sw_debug("PJSIP_INV_STATE_INCOMING");
      break;
    default:
      SIPPlugin::this_->sw_debug("{}, unhandled invite state", std::string(__FUNCTION__));
      break;
  }
}
...
void PJCall::call_on_media_update(pjsip_inv_session* inv, pj_status_t status) {
  ...
  if (status != PJ_SUCCESS) {
    SIPPlugin::this_->sw_warning("SDP negotiation failed");
    return;
  }
  ...
  if (call->ice_trans_send_ &&
    ...
    SIPPlugin::this_->sw_warning("ice negotiation as sender failed");
  if (call->ice_trans_ && !negotiate_ice(call->ice_trans_.get(), remote_sdp, inv->dlg->pool))
    SIPPlugin::this_->sw_warning("ice negotiation as receiver failed");
  // sending streams
  for (uint i = 0; i < call->media.size(); i++) {
    if (PJCallUtils::is_send_media(local_sdp->media[i])) {
      ...
      if (it == SIPPlugin::this_->sip_calls_->readers_.end()) {
        SIPPlugin::this_->sw_warning("no ShmdataToCb found for sending {} (PJCall)",
        ...
        call->media[i].cb_id = it->second->add_cb([=](void* data, size_t size) {
          ...
          SIPPlugin::this_->pjsip_->run([&]() {
            if (!call->ice_trans_send_->sendto(
              ...
              SIPPlugin::this_->sw_debug("error when sending data with ICE");
            }
          });
        });
      }
...
void PJCall::process_incoming_call(pjsip_rx_data* rdata) {
  ...
  if (!SIPPlugin::this_ || !SIPPlugin::this_->sip_calls_.get() ||
    ...
    SIPPlugin::this_->sw_warning("trying to initiate a call after all calls are hung out.");
    return;
  }
  ...
  SIPPlugin::this_->sw_debug("incoming call from {}", std::string(uristr, len));
  ...
  if (!SIPPlugin::this_->white_list_->is_authorized(peer_uri) &&
    ...
    SIPPlugin::this_->sw_debug("call refused from {}", peer_uri);
  ...
  if (rdata->msg_info.msg->body) {
    ...
    offer = sdp_info->sdp;
    if (nullptr == offer) SIPPlugin::this_->sw_warning("offer is null");
    ...
    if (status != PJ_SUCCESS) SIPPlugin::this_->sw_warning("bad SDP in incoming INVITE");
  }
  unsigned options = 0;
  status =
      pjsip_inv_verify_request(rdata, &options, nullptr, nullptr, PJSIP::this_->sip_endpt_, &tdata);
  if (status != PJ_SUCCESS) {
    SIPPlugin::this_->sw_warning("{}: can't handle incoming INVITE request",
  ...
  if (!call->ice_trans_) SIPPlugin::this_->sw_warning("ICE transport initialization failed");
  ...
  if (status != PJ_SUCCESS) {
    SIPPlugin::this_->sw_debug("error creating uas");
  ...
  if (PJ_SUCCESS != status) {
    SIPPlugin::this_->sw_warning("cannot answer to a call, it probably has too many streams");
    pjsip_response_addr res_addr;
    pjsip_get_response_addr(tdata->pool, rdata, &res_addr);
    pjsip_endpt_send_response(PJSIP::this_->sip_endpt_, &res_addr, tdata, nullptr, nullptr);
  }
...
void PJCall::print_sdp(const pjmedia_sdp_session* sdp) {
  ...
  if (len >= 1) {
    sdpbuf[len] = '\0';
    SIPPlugin::this_->sw_debug("SDP : \n{} \n ", std::string(sdpbuf));
  } else {
    SIPPlugin::this_->sw_error("error while printing SDP");
  }
}
...
bool PJCall::is_call_valid(const std::string& contact_uri) {
  ...
  if (SIPPlugin::this_->sip_presence_->sip_local_user_.empty()) {
    SIPPlugin::this_->sw_error("not registered to SIP server, call aborted");
    return false;
  }
  ...
  if (it != outgoing_call_.end()) {
    SIPPlugin::this_->sw_error("a call with {} already exists, new call aborted", contact_uri);
    return false;
  }
  ...
  if (sip_dest_user == std::string(sip_local_user, 0, sip_local_user.find_last_of(':'))) {
    SIPPlugin::this_->sw_error("cannot call self, call aborted");
    return false;
  }
  ...
  if (id == PJSUA_INVALID_ID) {
    SIPPlugin::this_->sw_error("cannot find buddy {},  call aborted", contact_uri);
    return false;
  }
...
pjsip_dialog* PJCall::create_sip_dialog(const pj_str_t local_uri, const pj_str_t remote_uri) {
  ...
  if (status != PJ_SUCCESS) {
    ...
    SIPPlugin::this_->sw_error("error while creating SIP dialog: {}", std::string(errstr));
    SIPPlugin::this_->sw_error("could not create SIP dialog, call aborted");
  ...
  if (status != PJ_SUCCESS) {
    char errstr[1024];
    pj_strerror(status, errstr, 1024);
    SIPPlugin::this_->sw_error("error while setting session credentials: {}", std::string(errstr));
    SIPPlugin::this_->sw_error("could not set session credentials, call aborted");
...
bool PJCall::send_invite_request(pjsip_dialog* dlg, call_t* call, const pjmedia_sdp_session* sdp) {
  ...
  if (status != PJ_SUCCESS) {
    char errstr[1024];
    pj_strerror(status, errstr, 1024);
    SIPPlugin::this_->sw_error("error while creating INVITE session: {}", std::string(errstr));
    SIPPlugin::this_->sw_error("could not create INVITE session, call aborted");
    pjsip_inv_terminate(call->inv, 500, PJ_FALSE);
    pjsip_dlg_terminate(dlg);
    outgoing_call_.pop_back();
    return false;
  ...
  if (status != PJ_SUCCESS) {
    char errstr[1024];
    pj_strerror(status, errstr, 1024);
    SIPPlugin::this_->sw_error("error while creating INVITE request: {}", std::string(errstr));
    SIPPlugin::this_->sw_error("could not create INVITE request, call aborted");
  ...
  if (status != PJ_SUCCESS) {
    char errstr[1024];
    pj_strerror(status, errstr, 1024);
    SIPPlugin::this_->sw_error("error while sending INVITE request: {}", std::string(errstr));
    SIPPlugin::this_->sw_error("could not send INVITE request, call aborted");
    pjsip_inv_end_session(call->inv, 500, nullptr, &tdata);
    pjsip_dlg_terminate(dlg);
    outgoing_call_.pop_back();
    return false;
...
bool PJCall::make_call(const std::string contact_uri) {
  ...
  if (!create_outgoing_sdp(dlg, cur_call, &sdp)) {
    SIPPlugin::this_->sw_error("could not create outgoing SDP, call aborted");
    pjsip_dlg_terminate(dlg);
    outgoing_call_.pop_back();
    return false;
  }
  ...
  if (!tree) {
    SIPPlugin::this_->sw_error("could not find buddy {} information tree, call aborted",
                               contact_uri);
    pjsip_tx_data* tdata = nullptr;
...
bool PJCall::create_outgoing_sdp(pjsip_dialog* dlg, call_t* call, pjmedia_sdp_session** res) {
  ...
  if (!call->ice_trans_send_) {
    SIPPlugin::this_->sw_error("could not initialize ICE transport for sending. Aborting call.");
    return false;
  }
  ...
  if (!desc.add_msg_attribute("ice-ufrag", std::string(ufrag_pwd.first.ptr, ufrag_pwd.first.slen))) {
    SIPPlugin::this_->sw_warning("could not add 'ice-ufrag' in outgoing SDP");
  }
  if (!desc.add_msg_attribute("ice-pwd", std::string(ufrag_pwd.second.ptr, ufrag_pwd.second.slen))) {
    SIPPlugin::this_->sw_warning("could not add 'ice-pwd' in outgoing SDP");
  }
  ...
  for (auto& path : paths) {
    ...
    if (!desc.add_media(media)) {
      SIPPlugin::this_->sw_warning("could not add media to the SDP description");
    } else {
      call->media.emplace_back();
  ...
  if (call->media.empty()) {
    SIPPlugin::this_->sw_error("no valid media could be added to SDP, call aborted");
    return false;
  }
  ...
  if (desc_str.empty()) {
    SIPPlugin::this_->sw_error("newly created SDP description is empty, call aborted");
    return false;
  }
  ...
  if (status != PJ_SUCCESS) {
    SIPPlugin::this_->sw_error("newly created SDP description is malformed, call aborted");
    return false;
  }
...
bool PJCall::send_to(const std::string& sip_url) {
  ...
  if (!can_create_calls_) {
    SIPPlugin::this_->sw_warning("trying to initiate a call after all calls have been hung out");
    return false;
  }
  if (sip_url.empty()) {
    SIPPlugin::this_->sw_warning("calling sip account received nullptr url");
    return false;
  }
  {
    std::unique_lock<std::mutex> lock(call_m_, std::defer_lock);
    if (!lock.try_lock()) {
      SIPPlugin::this_->sw_debug("cancel SIP send_to because an operation is already pending");
      return false;
    }
    is_calling_ = true;
...
bool PJCall::hang_up(const std::string& sip_url) {
  if (sip_url.empty()) {
    SIPPlugin::this_->sw_warning("hang up received nullptr url");
    return false;
  }
  {
    std::unique_lock<std::mutex> lock(call_m_, std::defer_lock);
    if (!lock.try_lock()) {
      SIPPlugin::this_->sw_debug("cancel SIP hang_up because an operation is already pending");
      return false;
    }
    is_hanging_up_ = true;
...
void PJCall::make_hang_up(pjsip_inv_session* inv) {
  ...
  if (status == PJ_SUCCESS && tdata != nullptr)
    ...
    SIPPlugin::this_->sw_warning("BYE has not been sent");
}
...
bool PJCall::attach_shmdata_to_contact(const std::string& shmpath,
  ...
  if (shmpath.empty() || contact_uri.empty()) {
    SIPPlugin::this_->sw_warning("cannot add shmpath for user (received nullptr)");
    return false;
  }
  SIPPlugin::this_->pjsip_->run([&]() {
...
void PJCall::make_attach_shmdata_to_contact(const std::string& shmpath,
  ...
  if (std::string("sip:") + contact_uri ==
    ...
    SIPPlugin::this_->sw_warning("cannot attach shmdata to self");
    return;
  }
  ...
  auto id = pjsua_buddy_find(&contact);
  if (PJSUA_INVALID_ID == id) {
    SIPPlugin::this_->sw_warning("buddy not found: cannot attach {} to {}", shmpath, contact_uri);
    return;
  }
  if (attach) {
    InfoTree::ptr tree =
  ...
  if (it == readers_.end()) {
    SIPPlugin::this_->sw_warning("error detaching a shmdata not attached (PJCall)");
    return;
  }
...
bool PJCall::negotiate_ice(PJICEStreamTrans* ice_trans,
  ...
  SIPPlugin::this_->sw_debug("ICE ufrag received: {}",
  ...
  SIPPlugin::this_->sw_debug("ICE pwd received: {}",
  ...
  for (unsigned i = 0; i < remote_sdp->media_count; ++i) {
    for (unsigned j = 0; j < remote_sdp->media[i]->attr_count; ++j) {
      if ("candidate" == std::string(remote_sdp->media[i]->attr[j]->name.ptr,
        ...
        ++cand_cnt;
        SIPPlugin::this_->sw_debug("ICE candidate received: {}",
        ...
        int cnt = sscanf(std::string(remote_sdp->media[i]->attr[j]->value.ptr,
                         ...
                         type);
        if (cnt != 7) {
          SIPPlugin::this_->sw_warning("error: invalid ICE candidate line");
          return false;
        }
        if (strcmp(type, "host") == 0)
          ...
          SIPPlugin::this_->sw_warning("error: invalid candidate type '{}'", std::string(type));
          return false;
        }
        ...
        if (PJ_SUCCESS != pj_sockaddr_set_str_addr(af, &cand->addr, &tmpaddr)) {
          SIPPlugin::this_->sw_warning("error: invalid IP address '{}'", std::string(ipaddr));
        }
  ...
  if (!ice_trans->start_nego(&ufrag->value, &pwd->value, cand_cnt, candidates)) {
    SIPPlugin::this_->sw_warning("error starting ICE negotiation");
    return false;
  }
  return true;
}
