...
/* Create transport for each media in the call */
    ...
    sip_instance_->install_method ("Call a contact",	//long name
				   ...
				   G_TYPE_BOOLEAN,
				   Method::
				   make_arg_type_description (G_TYPE_STRING,
							      nullptr), this);
    ...
    sip_instance_->install_property_by_pspec (sip_instance_->custom_props_->
					      get_gobject (),
					      rtp_session_name_spec_,
					      "rtp-session",
					      "Quiddity name managing RTP destinations");
    starting_rtp_port_spec_ =
      sip_instance_->custom_props_->make_int_property ("starting-rtp-port",
						       ...
						       G_PARAM_READWRITE,
						       PJCall::
						       set_starting_rtp_port,
						       PJCall::
						       get_starting_rtp_port,
						       this);
...
//FIXME 
  ...
  void
    PJCall::call_on_state_changed (pjsip_inv_session * inv, pjsip_event * e)
  {
  ...
  void
    PJCall::call_on_media_update (pjsip_inv_session * inv, pj_status_t status)
  {
    struct call *call;
	...
	status = stream_info_from_sdp (&current_media->si,
				       inv->pool,
				       med_endpt_, local_sdp, remote_sdp, i);
...
// testing if receiving
	if (PJMEDIA_DIR_DECODING == current_media->si.dir
	    ...
	    || PJMEDIA_DIR_CAPTURE_RENDER == current_media->si.dir)
	  {
		...
		if (0 ==
		    std::string (attr->name.ptr,
				 attr->name.slen).compare ("fmtp"))
		  {
...
//testing if fmtp line is about the considered media fmt
			if (0 ==
			    std::string (value, 0,
					 found).
			    compare (std::
				     to_string (current_media->si.fmt.pt)))
			  {
	...
	if (PJMEDIA_DIR_ENCODING == current_media->si.dir
	    ...
	    || PJMEDIA_DIR_CAPTURE_RENDER == current_media->si.dir)
	  {
	    QuiddityManager_Impl::ptr manager =
	      call->instance->sip_instance_->manager_impl_.lock ();
	    if ((bool) manager)
	      {
		{
		  manager->invoke (call->instance->rtp_session_name_,
				   "add_destination", nullptr,
				   {
				   call->peer_uri,
		  ...
		  manager->invoke (call->instance->rtp_session_name_,
				   "add_udp_stream_to_dest", nullptr,
				   {
				   current_media->shm_path_to_send,
				   call->peer_uri,
				   std::to_string (remote_sdp->media[i]->desc.
						   port)}
		  );
		}
	      }
...
/* Set the media as active */
  ...
  void PJCall::process_incoming_call (pjsip_rx_data * rdata)
  {
	   ...
	   media_index++)
	{
	  ...
	  for (unsigned int j = 0; j < offer->media[media_index]->attr_count;
	    ...
	    {
	      if (0 ==
		  pj_strcmp2 (&offer->media[media_index]->attr[j]->name,
			      "sendrecv")
		  || 0 ==
		  pj_strcmp2 (&offer->media[media_index]->attr[j]->name,
			      "sendonly"))
		{
		  tmp_media =
		    pjmedia_sdp_media_clone (dlg->pool,
	  ...
	  if (recv && nullptr != tmp_media)
		...
		(pjmedia_sdp_attr *) pj_pool_zalloc (dlg->pool,
						     sizeof
						     (pjmedia_sdp_attr));
	      std::string control_stream ("control:stream=" +
					  std::to_string (j));
			...
			pjmedia_sdp_session ** p_sdp)
  {
    ...
    sdp =
      (pjmedia_sdp_session *) pj_pool_zalloc (pool,
     ...
     */
    sdp->conn =
      (pjmedia_sdp_conn *) pj_pool_zalloc (pool, sizeof (pjmedia_sdp_conn));
...
// pj_strdup2 (pool, &attr->value, updated_val.c_str ()); 
  ...
  void PJCall::set_rtp_session (const gchar * quiddity_name, void *user_data)
  {
    ...
    if (!(bool) quid)
      {
	g_warning ("rtp manager %s not found", quiddity_name);
	context->rtp_session_name_.clear ();
	return;
      }

    context->rtp_session_name_ = quiddity_name;
    context->sip_instance_->custom_props_->notify_property_changed (context->
								    rtp_session_name_spec_);
  }

  const gchar *PJCall::get_rtp_session (void *user_data)
  {
    PJCall *context = static_cast < PJCall * >(user_data);
    return context->rtp_session_name_.c_str ();
  }

  Quiddity::ptr PJCall::retrieve_rtp_manager ()
  {
  ...
  gboolean PJCall::hang_up (gchar * sip_url, void *user_data)
  {
    ...
    context->sip_instance_->
      run_command_sync (std::
			bind (&PJCall::make_hang_up, context,
			      std::string (sip_url)));
  ...
  bool PJCall::make_hang_up (std::string contact_uri)
  {
	...
	if (0 == contact_uri.compare (app.call[i].peer_uri))
	  {
	    status =
	      pjsip_inv_end_session (app.call[i].inv, 603, nullptr, &tdata);
  ...
  void PJCall::set_starting_rtp_port (const gint value, void *user_data)
  {
    ...
    GObjectWrapper::notify_property_changed (context->sip_instance_->
