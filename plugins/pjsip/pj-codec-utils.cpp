...
/*       pad->static_caps.string);  */
	...
	PJCodecUtils::codecs from_caps =
	  inspect_rtp_codec_from_gst_caps (caps);
	...
	get_int_values_from_gst_struct (caps_struct, "payload");
      ...
      std::for_each (payloads.begin (),
		     ...
		     std::for_each (res.begin (),
				    ...
				    this_payload.
				    push_back (RTPCodec::
					       ptr (new RTPCodec (*codec)));
				    this_payload.back ()->payload_ =
				    std::move (pt);});
	...
	get_string_values_from_gst_struct (caps_struct, "media");
      ...
      std::for_each (media.begin (),
		     ...
		     std::for_each (res.begin (),
				    ...
				    {
				    this_media.
				    emplace_back (RTPCodec::
						  ptr (new
						       RTPCodec (*codec)));
				    this_media.back ()->media_ =
				    std::move (media_str);});
