    ...
    : Quiddity(std::forward<quiddity::Config>(conf), {kConnectionSpec}),
      port_id_(pmanage<MPtr(&property::PBag::make_string)>(
          ...
          [this](const std::string& valstr) {
            ...
            if (transport_id_ != -1) {
              LOGGER_DEBUG(this->logger, "SIP port changed for {}, closing existing SIP transport port {}", 
...
bool SIPPlugin::start_sip_transport() {
  ...
  LOGGER_DEBUG(this->logger, "Successfully started SIP transport on port {}", std::to_string(cfg.port));
  return true;
}
